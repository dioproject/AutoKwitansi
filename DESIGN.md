...
|--------|--------|----------|
| `commands.rs` | 39 command (kwitansi, produk, penjualan, backup, dashboard, POS, docs) | semua `cmd_*` |
...
- **POS Kasir** — katalog + keranjang + diskon/tunai/kembalian + tanggal bebas; simpan ke `penjualan`, cetak struk thermal, riwayat + edit + cetak ulang (tanpa sentuh kwitansi)
...
- Checkbox manual saling eksklusif (PPh 21 6% / PPh 21 5% narasumber / PPh 23 4% / PPh 23 2% sewa-jasa), tetap bisa diubah user.
- Bruto disimpan di `jumlah`; PPh 21 = 6% bruto, PPh 21 5% = 5% bruto, PPh 23 = 4% bruto, PPh 23 2% = 2% bruto; PPN = nominal opsional; total = bruto − PPh − PPN.
